...
//    std::mutex mutex_;
//    std::condition_variable cv_;        
//    bool quit_; // the mutex guards this, so going with non-atomic
//    
//    std::atomic<sig> state_;                    
//    std::thread thread_;
//
...
//        std::atomic<sig>& state_;
//        void make_available(std::memory_order mo = std::memory_order_release) { state_.store(sig::available,mo); }                         
//        friend void make_available(availability_request& p, std::memory_order mo = std::memory_order_release) noexcept {            
//            p.make_available(mo);     
//        }
//    };
//    
//    struct consumable_state {
//        std::atomic<sig>& state_;
//        std::condition_variable& cv_;                    
//        void consume(std::memory_order mo = std::memory_order_release) { state_.store(sig::consumed,mo); cv_.notify_one(); }
//        friend void consume(consumable_state& p, std::memory_order mo = std::memory_order_release) { 
//            p.consume(mo);
//        }
//    };
//    
...
//                if (quit_) { return; }                
//
//                func(availability_request{state_});                
//            }                                        
//        } catch(...) {            
//            exception_handler();
//        }
...
//    work_thread(F&& func, H&& exception_handler)
//        :   quit_{false},
//            state_{sig::initializing}            
//    {
//        std::promise<void> prom_;
//        std::future<void> started_ = prom_.get_future();
//        thread_ = std::thread{&work_thread::dowork<std::remove_reference_t<F>,std::remove_reference_t<H>>,
//                              this,std::forward<F>(func),std::forward<H>(exception_handler),std::move(prom_)};        
//        started_.get();
//        std::unique_lock<std::mutex> lock_{mutex_};
//        // we are now synchronized with the waiting condition_variable
//    }    
...
//        switch(s) {
//            case sig::available     : func(consumable_state{state_, cv_}); 
//                                      state_.store(sig::consumed, std::memory_order_release);
//                                      cv_.notify_one();
//                                      return;
//
//            case sig::initializing  : gunc();
//                                      state_.store(sig::consumed, std::memory_order_release);                                      
//                                      cv_.notify_one();                                      
//                                      return;  
//
...
//    
//    template <typename F>
//    void block_and_call(F&& func) {
//        std::unique_lock<std::mutex> lock_{mutex_}; // HACK lock mutex; presume we are waiting in the work thread; could be cleaned up a bit to verify this
//        as func(consumable_state{state_,cv_});  // call func with consumable state.. No guarantee that it is consumable...
//    }
//
//    ~work_thread() 
//    {                    
...
//           state_.store(sig::consumed, std::memory_order_release);            
//           cv_.notify_one();  
//        }  // lock is destroyed, thread can now advance..        
//           if (thread_.joinable()) {
//               thread_.join();
//           }
//    }
