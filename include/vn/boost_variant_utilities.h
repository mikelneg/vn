...
{
    using ret_var =
        typename type_list<std::remove_reference_t<AdditionalTypes>..., std::remove_reference_t<Args>...>::unique::template discharge<boost::variant>;
...
//    using ret_var = typename type_list<std::remove_reference_t<AdditionalTypes>...,std::remove_reference_t<Args>...>::unique::template
//    discharge<boost::variant>;
...
struct lambda_visitor : V {
    ...
    constexpr lambda_visitor(Ts&&... ts) noexcept(noexcept(V(std::forward<Ts>(ts)...))) : V(std::forward<Ts>(ts)...)
    {
    }
