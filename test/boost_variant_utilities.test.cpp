...
struct foo {
    friend std::size_t hash_value(foo const&)
    {
        return 55;
    }
};
...
TEST_CASE("boost_variant_utilities", "[boost_variant_utilities]")
{
    ...
    SECTION("vn::make_lambda_visitor tests")
    {
        ...
        {
            int i{};
            auto lv = make_lambda_visitor([&](int) { ++i; }, [&](foo&) { ++i; }, [&](char) { ++i; });
            for (auto&& e : vv)
            {
                apply_visitor(lv, e);
            }
        ...
        {
            int i{};
            auto lv = make_lambda_visitor([&](auto&) { ++i; }, [&](int) { i += 2; });
        ...
        {
            int i{};
            auto lv = make_lambda_visitor([&](int) { i += 2; }, [&](auto&) { ++i; });
        ...
        {
            int i{};
            auto lv = make_lambda_visitor([&](int) { ++i; }, [&](char) { ++i; }, [&](foo&) { ++i; });
